...
import { Copy, Check, X, ChevronRight, ExternalLink, Play } from 'lucide-react';
...
export function ProgressDoc() {
  ...
  const jsxCode = `import { Progress } from './components/wugweb/Progress';

// Default Size
<Progress value={33} showLabel={true} />

// Small Size
<Progress value={66} height={4} />

// Large Size
<Progress value={90} height={12} />

// Design Tokens Used:
// Track: var(--surface-700)
    ...
    <div className="stagger-children" style={{ display: 'flex', flexDirection: 'column', gap: sectionGap, position: 'relative', width: '100%', maxWidth: '100%', boxSizing: 'border-box', overflowX: 'hidden' }}>
      ...
      <section className="animate-fade-in-up" style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '24px', width: '100%', boxSizing: 'border-box' }}>
        <nav style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: 'var(--text-sm)', color: 'var(--muted-foreground)', width: '100%', boxSizing: 'border-box' }}>
          ...
          <span>Feedback</span>
          <ChevronRight size={16} />
          <span style={{ color: 'var(--foreground)' }}>Progress</span>
        </nav>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: isMobile ? '16px' : '32px', padding: isMobile ? '16px' : '20px', background: 'var(--background)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
              <div>
                ...
                <div style={{ fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-semibold)' }}>Labeled</div>
              </div>
              <div>
                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted-foreground)', marginBottom: '4px' }}>Tokens</div>
                <div style={{ fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-semibold)' }}>Accent</div>
              </div>
              <div>
                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted-foreground)', marginBottom: '4px' }}>Animation</div>
                <div style={{ fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-semibold)' }}>Smooth</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--foreground)' }}>Live Demo</h3>
                  <Button size="sm" onClick={simulateProgress} style={{ gap: '8px', background: 'var(--accent)', color: 'var(--accent-foreground)' }}>
                    <Play size={14} />
                    Simulate Load
                  </Button>
                </div>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', padding: '32px', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', background: 'var(--surface-800)' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: 'var(--text-sm)', color: 'var(--foreground)' }}>Default Size (with Label)</label>
                    <Progress value={value} showLabel />
                  </div>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: 'var(--text-sm)', color: 'var(--foreground)' }}>Large Size</label>
                    <Progress value={value} height={12} />
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: 'var(--text-sm)', color: 'var(--foreground)' }}>Small Size</label>
                    <Progress value={value} height={4} />
                  </div>
                </div>
              </div>
      ...
      <section className="animate-fade-in-up" style={{ animationDelay: '200ms', width: '100%', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: '16px', width: '100%' }}>
          <TokenCard label="Track Color" token="--surface-700" value="#262626" color="var(--surface-700)" onClick={() => handleTokenClick('--surface-700', 'Track Color', '#262626')} isHighlighted={highlightedToken === '--surface-700'} />
          <TokenCard label="Indicator" token="--accent" value="#FFBE1A" color="var(--accent)" onClick={() => handleTokenClick('--accent', 'Indicator', '#FFBE1A')} isHighlighted={highlightedToken === '--accent'} />
          <TokenCard label="Radius" token="--radius-full" value="9999px" isRadius onClick={() => handleTokenClick('--radius-full', 'Radius', '9999px')} isHighlighted={highlightedToken === '--radius-full'} />
        </div>
      </section>
                ...
                <div style={{ display: 'flex', gap: '12px', padding: '16px', background: 'var(--background)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                  ...
                  <div>
                    ...
                    <ul style={{ margin: 0, paddingLeft: '20px', fontSize: 'var(--text-sm)', color: 'var(--muted-foreground)', lineHeight: 1.6 }}>
                      <li>Use standard <code>progressbar</code> role implicitly or explicitly</li>
                      <li>Ensure color contrast between indicator and track is sufficient</li>
                    </ul>
                  </div>
                </div>
              </div>
